...
std::vector <std::pair<image_pair_t, std::vector < cv::DMatch>>>

Database::ReadAllMatches() const {
...
void Database::save(const Frame *frame) {
    BeginTransaction();

    Database::Image db_image;
    if(!ExistImageByName(frame->imageName)) {
        db_image.id = frame->id;
        db_image.name = frame->imageName;
        WriteImage(db_image, true);
    }

    WriteKeyPoints(frame->id, frame->kp);
    WriteDescriptors(frame->id,frame->desp);
    WriteKeyPointsColor(frame->id,frame->color);
